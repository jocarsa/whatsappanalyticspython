    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        for line in file:
            ...
            if match:
                ...
                parsed_messages.append({"date": date, "hour": time, "name": name, "message": message})
...
def plot_messages_pie_chart(parsed_messages, output_file):
    ...
    labels, sizes = name_counts.keys(), name_counts.values()
...
def display_charts():
    ...
    def delayed_cleanup(folder):
        time.sleep(15)  # Delay for 5 seconds
        if os.path.exists(folder):
            shutil.rmtree(folder)

    threading.Thread(target=delayed_cleanup, args=(user_output_folder,)).start()
